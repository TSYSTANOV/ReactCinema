...
function Categories(){
    ...
    useEffect(()=>{
        ...
        async function getData(){
            if(searchParam === 'triends'){
                setTitle('В тренде')
                const {results} = await API_component.getTrending()
                setList(results)
            }else{
                const [type, media_type] = searchParam.split('-')
                const {results} = await API_component.getData(media_type, type === 'top' ? 'top_rated': type)
    ...
    return <section className="other-films">
